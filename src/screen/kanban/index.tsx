...
export const useDragEnd = ()=>{
    ...
    const {mutate:reorderKanban} = useRecorderKanban(useKanbansQueryKey());
    const {mutate:recorderTask} = useRecorderTask(useTasksQueryKey());
    ...
    return useCallback(({source,destination,type}:DropResult)=>{
        ...
        if(type === 'COLUMN'){
            ...
            reorderKanban({fromId,referenceId:Number(toId),type})
        }
        if(type === 'ROW'){
    ...
    },[allTasks,kanbans,reorderKanban,recorderTask])
}